@extends('layouts.app')
@section('title', 'Bagian Handprint')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="flex justify-between items-center px-6 py-4 border-b">
                <h2 class="text-xl font-bold">User Handprint</h2>
            </div>
            <div class="px-6 py-4">
                @if (Auth::user()->role == 'monitoring')
                    <form action="{{ route('handprint.storeHandprint', ['bagian' => 'handprint']) }}" method="POST"
                        class="space-y-4">
                    @elseif(Auth::user()->role == 'user')
                        <form action="{{ route('user.handprint.storeHandprint', ['bagian' => 'handprint']) }}"
