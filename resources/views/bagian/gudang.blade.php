        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            ...
            <div class="px-6 py-4">
                @if (Auth::user()->role == 'monitoring')
                    <form action="{{ route('gudang.storeGudang', ['bagian' => 'gudang']) }}" method="POST"
                        class="space-y-4">
                    @elseif(Auth::user()->role == 'user')
                        <form action="{{ route('user.gudang.storeGudang', ['bagian' => 'gudang']) }}" method="POST"
                            class="space-y-4">
                @endif
                @csrf
                ...
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <div>
                        <label for="aktual_permintaan_packing" class="block text-sm font-medium text-gray-700 mb-1">Aktual
                            Permintaan Packing</label>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label for="keterangan_reject" class="block text-sm font-medium text-gray-700 mb-1">Dari
                            Pagi</label>
                        <textarea name="keterangan_reject" id="keterangan_reject" cols="30" rows="3"
                            class="w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required></textarea>
                    </div>
                    <div>
                        <label for="keterangan_rework" class="block text-sm font-medium text-gray-700 mb-1">Dari
                            Siang</label>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const smvInput = document.getElementById('aktual_permintaan_packing');
