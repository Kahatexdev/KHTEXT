                        ...
                        <div class="min-w-full">
                            ...
                            <div class="block sm:hidden">
                                @foreach ($data as $item)
                                    <div
                                        class="border-b border-gray-100 p-4 hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center space-x-3">
                                                <div
                                                    class="w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 flex items-center justify-center text-white font-semibold text-sm">
                                            ...
                                            <div class="flex items-center space-x-2">
                                                {{-- edit --}}
                                                <button type="button"
                                                    class="editBtn p-2 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white rounded-lg transition-all duration-200 shadow-sm hover:shadow-md"
                                                    data-nama="{{ $item->nama_area }}"
                                                    data-url="{{ route('area.edit', $item->nama_area) }}">
                                        ...
                                        <tr
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <div class="mr-4">
                                                        <div
                                                            class="w-10 h-10 rounded-xl bg-gradient-to-r from-gray-100 to-gray-200 flex items-center justify-center">
                                                    ...
                                                    <div>
                                                        <div class="text-sm font-semibold text-gray-900">
                                                            {{ $item->nama_area }}</div>
                                                        <div class="text-xs text-gray-500">Area ID: {{ $item->nama_area }}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                                <div class="flex items-center justify-end space-x-2">
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        data-nama="{{ $item->nama_area }}"
                                                        data-url-fetch="{{ route('area.edit', $item->nama_area) }}"
                                                        data-url-update="{{ route('area.update', $item->nama_area) }}">
                                                        <div
                                                            class="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                                        </div>
        ...
        $(function() {
            ...
            function showErrors(errs) {
                ...
                $.each(errs, (f, msgs) => {
                    ...
                    $form.find('[name="' + f + '"]').removeClass('border-gray-200').addClass(
                        'border-red-500');
                });
            }
            ...
            searchBox.on('input', function() {
                ...
                searchTimeout = setTimeout(() => {
                    $('tbody tr, .block.sm\\:hidden > div').each(function() {
                        const areaName = $(this).find('td:nth-child(2), h4').text()
                            .toLowerCase();
            ...
            $(document).on('click', '.editBtn', function() {
                // grab the two URLs + the name
                const nama = $(this).data('nama_area');
                const fetchUrl = $(this).data('url-fetch');
                const updateUrl = $(this).data('url-update');

                clearErrors();
                title.text(`Edit Area: ${nama}`);

                // set the form up for PUT against the correct URL
                $form.attr('action', updateUrl);
                // ensure we only append one _method override
                $form.find('input[name="_method"]').remove();
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $form.find('input[name="nama_area"]').val(data.nama_area);
                        openModal();
                    },
                    error: function() {
                        Swal.fire({
                            title: 'Gagal!',
                            text: 'Terjadi kesalahan saat memuat data area.',
                            icon: 'error',
                            confirmButtonColor: '#ef4444'
                        });
                    }
